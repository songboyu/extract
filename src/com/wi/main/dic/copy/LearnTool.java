...
import com.wi.main.recognition.ForeignPersonRecognition;
import com.wi.main.recognition.RuleRecognition;
import com.wi.main.util.Graph;
import com.wi.tire.domain.SmartForest;
import com.wi.tire.util.CollectionUtil;
...
public class LearnTool {
	...
	private void findAsianPerson(Graph graph) {
		List<NewWord> newWords = new AsianPersonRecognition(graph.terms)
				.getNewWords();
		addListToTerm(newWords);
	}

	private void findForeignPerson(Graph graph) {
		List<NewWord> newWords = new ForeignPersonRecognition(graph.terms)
				.getNewWords();
		addListToTerm(newWords);
	}
	...
	private void valueResult(SmartForest<NewWord> smartForest,
			HashMap<String, Double> hm, Nature nature) {
		// TODO Auto-generated method stub
		...
		for (int i = 0; i < smartForest.branches.length; i++) {
			...
			if (smartForest.branches[i].getStatus() == 3) {
				if (param.isActive()
						&& (nature == null || param.getNature().equals(nature))) {
