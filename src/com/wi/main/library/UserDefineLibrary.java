...
import static com.wi.main.util.MyStaticValue.LIBRARYLOG;
...
import com.wi.tire.domain.Value;
import com.wi.tire.domain.WoodInterface;
import com.wi.tire.library.Library;
	...
	private static void initAmbiguityLibrary() {
		...
		if (file.isFile() && file.canRead()) {
			try {
				...
				LIBRARYLOG.warning("init ambiguity  error :"
			...
			LIBRARYLOG.warning("init ambiguity  waring :"
					+ new File(ambiguityLibrary).getAbsolutePath()
					+ " because : not find that file or can not to read !");
		}
	}
		...
		try {
			...
			while ((temp = br.readLine()) != null) {
				if (StringUtil.isBlank(temp)) {
					...
					// 如何核心辞典存在那么就放弃
					if (MyStaticValue.isSkipUserDefine
							&& InitDictionary.getWordId(strs[0]) > 0) {
						continue;
					}
	...
	public static void loadLibrary(Forest forest, String path) {
		...
		if (path != null) {
			...
			if (!file.canRead() || file.isHidden()) {
				LIBRARYLOG.warning("init userLibrary  waring :"
			...
			if (file.isFile()) {
				...
				LIBRARYLOG.warning("init user library  error :"
						+ new File(path).getAbsolutePath()
