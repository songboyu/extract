package com.wi.main.app.crf.model;
...
import com.wi.main.app.crf.Model;
import com.wi.main.app.crf.pojo.Feature;
import com.wi.main.app.crf.pojo.Template;
import com.wi.tire.util.IOUtil;
import com.wi.tire.util.StringUtil;
...
public class WapitiCRFModel extends Model {
	...
	private void parseFile(String path, String templatePath) throws Exception {
		...
		// read config
		String content = IOUtil.getContent(IOUtil.getReader(templatePath,
				IOUtil.UTF8));

		this.template = (Template) Template.parse(content);
		...
		for (String str : statusLines) {
			String[] split = str.split("\t");
			status[statusMap.get(split[1])][statusMap.get(split[2])] = Double
					.parseDouble(split[3]);
		}
		IOUtil.close(reader);
	...
	private String fixName(String substring) {
		...
		for (String string : split) {
			if (string.startsWith("_x")) {
				string = String.valueOf((char) ('B' + Integer.parseInt(string
						.substring(2))));
			}
			sb.append(string);
		}
	 ...
	 * @throws Exception
	 */
	public static Model parseFileToModel(String modelPath, String templatePath,
			int maxSzie) throws Exception {
